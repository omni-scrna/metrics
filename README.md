...
cell-type annotation, PCA embedding, or KNN neighbor graph) plus a ground-truth cell-type
labels TSV, aligns rows by `cell_id`, computes its fixed metric suite, and writes one JSON
scores file (`<name>_<stage>_metrics.json`) to `--output_dir`.
